...
fn main() {
    let mut niteUserTracker: MaybeUninit<NiteUserTracker> =
        MaybeUninit::<NiteUserTracker>::uninit();

    let mut userTracker: nite_UserTracker = nite_UserTracker {
        m_userTrackerHandle: &mut niteUserTracker,
    };
    ...
    while (!wasKeyboardHit()) {
        ...
        let users = unsafe {
            std::slice::from_raw_parts(
                userTrackerFrameRef.as_ref().unwrap().m_users.m_data,
                userTrackerFrameRef.as_ref().unwrap().m_users.m_size as usize,
            )
        };
        for i in 0..users.len() {
            let user = users[i];

            updateUserState(user._base, unsafe {
                (*(userTrackerFrameRef))
                    .m_pFrame
                    .as_mut()
                    .unwrap()
            ...
            if user._base.state & 2 != 0 {
                ...
                if head.positionConfidence > 0.5 {
                    println!(
                        "{}. ({}, {}, {})",
                        user._base.id, head.position.x, head.position.y, head.position.z
                    );
                } else {
